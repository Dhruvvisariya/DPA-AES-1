...
class RealisticAESTraceGenerator:
    ...
    def _sub_bytes(self, state):
        ...
        for i in range(16):
            ...
            power_trace.extend(self._generate_operation_samples(hamming_weight, amplitude=1.2))
    ...
    def _shift_rows(self, state):
        ...
        for i in range(1, 4):
            ...
            power_trace.extend(self._generate_operation_samples(0.5))
    ...
    def _mix_columns(self, state):
        ...
        for col in range(4):
            ...
            power_trace.extend(self._generate_operation_samples(hamming_distance, amplitude=1.5))
    ...
    def _generate_operation_samples(self, base_power, amplitude=1.0, num_samples=None):
        ...
        # Create bell curve for power consumption
        x = np.linspace(-2, 2, num_samples)
        power_curve = amplitude * base_power * np.exp(-x**2)
    ...
    def generate_trace(self, plaintext):
        ...
        trace, state = self._add_round_key(state, self.key)  # Simplified: should be final round key
        full_trace.extend(trace)
...
def save_traces(traces, plaintexts, ciphertexts, key, base_filename):
    """Save traces in CSV format optimized for DPA"""
    # Save traces
    ...
    metadata = {
        ...
        'key': [key.hex()] * len(traces)  # Include key for validation
    }
...
def main():
    # Parameters
    num_traces = 100
    samples_per_cycle = 1 # Adjust for more/fewer points per operation
    ...
    generator = RealisticAESTraceGenerator(samples_per_cycle=samples_per_cycle)
    ...
    plt = visualize_trace(traces[0], "Single AES Encryption Power Trace")
    plt.savefig(f'{base_filename}_example_trace.png')
    plt.close()
    
    print("\nFiles saved:")
    print(f"- Traces: {base_filename}_traces.csv")
    print(f"- Metadata: {base_filename}_metadata.csv")
    print(f"- Example visualization: {base_filename}_example_trace.png")
